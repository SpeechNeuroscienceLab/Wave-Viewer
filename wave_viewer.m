...
%   or: 'event_params',<an event_params struct from a viewer_end_state>
  ...
  plot_params    = struct('hzbounds4plot', [], ...
                                 'name', 'signal', ...
                                 'axfracts', [], ...
                                 'yes_gray', 1, ...
                                 'thresh_gray', 0, ...
  ...
  params.plot_params = plot_params;
  params.sigproc_params = sigproc_params;
  ...
  if axinfo.yes_add_user_events
    for iax = 1:nax
      ...
      if axinfo.yes_add_user_events
        ...
        %move2front(ax(iax),axinfo.h_tmarker_spec); % move2front doesn't work in Matlab 2023 but appears to be a minor graphics adjustment that we can remove JLG 7/18/24
